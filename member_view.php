...
<body>
    ...
    if (isset($_POST['delete'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM UserClubs WHERE UFID = ? AND ClubID = ?");
        $stmt->bind_param("ii", $deleteId, $clubId);

        if ($stmt->execute()) {
            echo "<script>alert('Member removed successfully'); location.reload();</script>";
        } else {
    ...
    if(isset($_GET['session']) && isset($_GET['club'])) {
        ...
        if ($isPres) {
            ...
            echo "<form name='make-event-form'><input type='text' id='event-name-input' placeholder='event name'><input type='date' id='event-date-input'><input type='text' id='event-code-input' placeholder='CODE'></form></div>";
            echo "<div class='remove-member-box'><table class='remove-member-table'>";
                ...
                while ($row = $mems->fetch_assoc()) {
                    ...
                    if ($usr->num_rows > 0) {
                        while ($usrRow = $usr->fetch_assoc()) {
                            ...
                            echo "<td>
                                <form method='post' action='member_view.php'>
                                    <input type='hidden' name='deleteId' value='" . $usrRow["UFID"] . "'>
                                    <input type='hidden' name='clubId' value='" . $club_id . "'> <!-- Add hidden input for club_id -->
